...
export async function fetchHAEntities(
  ...
  try {
    ...
    // Pass HA config to backend via query params (backend needs to know where to connect)
    if (config.host) url.searchParams.set('host', config.host)
    if (config.port) url.searchParams.set('port', String(config.port))
    if (config.protocol) url.searchParams.set('protocol', config.protocol)
    
    const response = await fetch(url.toString(), {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'X-HA-Token': token, // Backend will use this to authenticate with HA
      },
    })
 ...
 * @param filter Comma-separated domain list (e.g., "sensor,input_number")
 */
export function filterEntitiesByDomain(
  ...
  const domains = filter.split(',').map(d => d.trim().toLowerCase())
